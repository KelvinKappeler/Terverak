...
object CardDescriptionView {
    ...
    val descriptionOnlyMinion =
      card match {
        case minion: Card.MinionCard =>
          Batch(
            Group(Text("Minion attributes:", description_x, description_y + (8 + Math.max(card.effectsWhenDiscard.length, 1)) * textOffset, baseDepth, GameAssets.Fonts.fontNormal8Key, GameAssets.Fonts.fontNormal8Material.withTint(RGBA.Purple))).withDepth(Depth(baseDepth)),
  ...
  private def computeBatchForEffectsAttributesDescription(list: List[_], offsetY: Int): Batch[SceneNode] = {
    ...
    if (list.isEmpty) Batch(Group(Text("None", description_x, description_y + offsetY * textOffset, baseDepth, GameAssets.Fonts.fontNormal8Key, GameAssets.Fonts.fontNormal8Material.withTint(RGBA.White))).withDepth(Depth(baseDepth)))
    else rec(list, 0)
  }
}
