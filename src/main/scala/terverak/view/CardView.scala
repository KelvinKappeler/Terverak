    ...
    if (cardViewModel.isRevealed) {
      val batch: Batch[Graphic[_]] =
        if (!cardViewModel.isSmallVersion) {
          ...
          Batch(Graphic(x, y, cardWidth, cardHeight, depth, Material.Bitmap(card.imageName)).withScale(CardViewModel.SmallVersionScale))
        }
      ...
      if (!cardViewModel.isSmallVersion) {
        ...
        Batch(Graphic(x, y, cardWidth, cardHeight, depth, Material.Bitmap(GameAssets.Cards.cardBack)).withScale(CardViewModel.SmallVersionScale))
      }
    }
  }
}
